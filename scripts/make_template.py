...
处理步骤:
  ...
  3. 替换汇报人姓名等隐私信息为 "xxx"
  4. 内容页文本替换为占位符（此处填充文本/图片），按结构去重
  5. 按页面类型去重（SECTION 保留首个，其余类型仅保留首个）

用法: python make_template.py <input.pptx> [output.pptx]
输出: 保留设计骨架的模板 PPTX（含去重后的内容页）
"""
...
def _content_fingerprint(slide):
    """生成内容页的结构指纹，用于去重。

    指纹包含：形状类型序列、各形状的位置/尺寸区间、文本框数量。
    结构相同的内容页只保留第一个。
    """
    shape_types = []
    positions = []  # (l_bucket, t_bucket, w_bucket, h_bucket)
    has_text = 0
    has_picture = 0
    for shape in slide.shapes:
        st = str(shape.shape_type)
        ...
        # 位置/尺寸离散化（0.5in 粒度）
        l = int(shape.left / 914400 / 0.5)
        t = int(shape.top / 914400 / 0.5)
        w = int(shape.width / 914400 / 0.5)
        h = int(shape.height / 914400 / 0.5)
        positions.append((l, t, w, h))
        if shape.has_text_frame and shape.text_frame.text.strip():
            has_text += 1
        if 'PICTURE' in st:
            has_picture += 1
    return (tuple(shape_types), tuple(sorted(positions)), has_text, has_picture)


def trim_slides(prs):
    """
    去重幻灯片：
      - COVER / TOC / THANKS：每种仅保留首次出现的那个
      - SECTION_XX：保留第一个（构建时按需自动克隆）
      - 内容页（CONTENT / CONTENT_FRAME / DATA / DISCUSSION）：
        保留但去重 —— 结构指纹相同的只保留第一个，多余删除
